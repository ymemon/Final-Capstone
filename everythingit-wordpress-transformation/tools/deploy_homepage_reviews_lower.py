...
if(substr_count($reviewsHtml,'class="eit-google-card"')!==10){{throw new RuntimeException('Expected 10 review cards');}}
if(substr_count($reviewsHtml,'aria-label="Review ')!==10){{throw new RuntimeException('Expected 10 review controls');}}
...
echo wp_json_encode(['page_id'=>$id,'backup'=>basename($backup),'iso_sections'=>substr_count($render,'class="eit-iso-modern"'),'review_sections'=>substr_count($render,'class="eit-google-reviews"'),'cards'=>substr_count($render,'class="eit-google-card"'),'controls'=>substr_count($render,'aria-label="Review ')]);
'''


def main() -> None:
    source = SOURCE.read_text(encoding="utf-8-sig")
    if source.count('class="eit-google-card"') != 10 or source.count('aria-label="Review ') != 10:
    ...
    checks={'cards':response.text.count('class="eit-google-card"'),'controls':response.text.count('aria-label="Review '),'iso':response.text.count('class="eit-iso-modern"'),'reviews':response.text.count('class="eit-google-reviews"'),'tablet_frame':'grid-column:1/-1' in response.text}
    print(json.dumps({'status':response.status_code,'public':checks}))
    if checks!={'cards':10,'controls':10,'iso':1,'reviews':1,'tablet_frame':True}:raise RuntimeError(f'Public verification failed: {checks}')
