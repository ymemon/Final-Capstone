...
/** How many of the page's own links are actually followed and checked. */
define( 'AZWC_LINKCHECK_MAX', 10 );
/** Per-link timeout. Short on purpose — one slow third-party site should not stall the whole audit. */
define( 'AZWC_LINKCHECK_TIMEOUT', 6 );
/** How old a sitemap's newest <lastmod> can be before it is flagged as stale. */
...
/**
 * Same private-network guard as azwc_audit_normalize(), reused for every
 * link the broken-links check is about to probe.
 *
 * Without this, the SSRF protection on the main URL would be pointless: an
 * attacker submits a page they control, that page links to
 * http://169.254.169.254/ or an internal admin panel, and this server
 * dutifully fetches it on their behalf while checking "does this link work".
 */
...
/**
 * HEAD first (cheap), falling back to a capped GET when a server refuses
 * HEAD outright (405/501) or simply doesn't answer it — some do neither
 * correctly and just hang up, which reads here as status 0.
 */
...
function azwc_audit_plain_english( $id ) {
	static $map = array(
		...
		'broken_links'        => 'A broken link points at a page that no longer loads - a moved page, a typo in the address, or something taken down elsewhere. A visitor who clicks it hits a dead end and leaves, and a search engine that follows it wastes crawl budget on a page that goes nowhere.',
		'structured_data_valid' => 'Structured data has to be syntactically valid JSON, or search engines silently discard the entire block - a single missing comma or stray quote throws away every rich-result feature it was meant to add, with no visible symptom on the page itself.',
		'sitemap_freshness'   => 'The lastmod date in a sitemap tells search engines when a page last changed, so they know whether it is worth re-crawling. A sitemap that never updates teaches Google to check back less and less often, even while the real site keeps changing underneath it.',
		'hreflang'            => 'hreflang tags tell Google which version of a page to show a visitor based on their language or country - a UK page instead of a US one, for example. They only matter once more than one version of a page exists; a single-language site needs none of this.',
	);
...
function azwc_audit_check( $id, $label, $status, $detail, $weight = 1, $group = 'technical', $items = array() ) {
	// Looked up rather than passed in, so every call site stays untouched
...
function azwc_audit_run_checks( $url, $page, $chain ) {
	...
	/**
	 * Sitemap freshness, from the <lastmod> dates already in hand.
	 *
	 * Reads the same fetched sitemap body rather than a second request. A
	 * sitemap index's own <lastmod> values (when present) describe when each
	 * child sitemap was regenerated, which is still a fair freshness signal
	 * even though the page-level dates live one level deeper.
	 */
	...
	if ( $sitemap_lastmods ) {
		...
		$checks[] = azwc_audit_check(
			...
			$future
				? sprintf( 'The newest <lastmod> date in the sitemap is %s, which is in the future. Worth checking the server clock or whatever generates the sitemap.', gmdate( 'Y-m-d', $newest ) )
				: sprintf(
					'The newest <lastmod> date in the sitemap is %s (%d day%s ago).%s',
					gmdate( 'Y-m-d', $newest ),
					$abs,
					1 === $abs ? '' : 's',
					$abs > AZWC_SITEMAP_STALE_DAYS ? ' A sitemap this stale often means it is not being regenerated automatically as the site changes.' : ''
				),
			1,
			'indexability'
		);
		...
		$checks[] = azwc_audit_check(
			...
			'The sitemap reflects recent activity',
			'info',
			'The sitemap does not include <lastmod> dates, so freshness cannot be measured from it.',
			1,
			'indexability'
		);
	}
	...
	foreach ( $ld[1] as $block ) {
		...
		// json_decode('null') legitimately returns NULL with no error, so the
		// error code is checked directly rather than inferring failure from
	...
	$ld_total = count( array_filter( $ld[1], function ( $b ) { return '' !== trim( $b ); } ) );
	if ( $ld_total > 0 ) {
		$checks[] = azwc_audit_check(
			...
			empty( $ld_invalid )
				? sprintf( 'All %d JSON-LD block%s on the page parsed as valid JSON.', $ld_total, 1 === $ld_total ? '' : 's' )
				: sprintf(
					'%d of %d JSON-LD block%s failed to parse as valid JSON, so search engines discard %s entirely rather than reading it with an error.',
					count( $ld_invalid ),
					$ld_total,
					1 === $ld_total ? '' : 's',
					1 === count( $ld_invalid ) ? 'it' : 'them'
				),
			2,
			'structured',
			$ld_invalid
	...
	// hreflang: attribute order on <link rel="alternate" hreflang="..."> is
	// not guaranteed, so each candidate tag is matched on both attributes
	// independently rather than with one fixed-order pattern.
	preg_match_all( '#<link\b[^>]*>#i', $html, $linktags );
	$hreflangs_raw = array();
	foreach ( $linktags[0] as $tag ) {
		if ( preg_match( '#\brel\s*=\s*["\']alternate["\']#i', $tag )
			&& preg_match( '#\bhreflang\s*=\s*["\']([^"\']+)["\']#i', $tag, $hfm ) ) {
			$hreflangs_raw[] = strtolower( trim( $hfm[1] ) );
		}
	}
	$hreflang_counts = array_count_values( $hreflangs_raw );
	$hreflang_dupes  = array_keys( array_filter( $hreflang_counts, function ( $n ) { return $n > 1; } ) );

	if ( $hreflangs_raw ) {
		$checks[] = azwc_audit_check(
			'hreflang',
			'Language/region targeting tags (hreflang)',
			$hreflang_dupes ? 'warn' : 'pass',
			$hreflang_dupes
				? sprintf(
					'%d hreflang tag%s found, but %s repeated: %s. When the same language/region value appears more than once, Google ignores the whole hreflang set on this page.',
					count( $hreflangs_raw ),
					1 === count( $hreflangs_raw ) ? '' : 's',
					1 === count( $hreflang_dupes ) ? 'one value is' : 'some values are',
					implode( ', ', $hreflang_dupes )
				)
				: sprintf(
					'%d hreflang tag%s found, targeting: %s.',
					count( $hreflangs_raw ),
					1 === count( $hreflangs_raw ) ? '' : 's',
					implode( ', ', array_unique( $hreflangs_raw ) )
				),
			1,
			'structured'
		);
	...
	/**
	 * Broken links — actually requesting a sample of what the page points at.
	 *
	 * Every href is deduplicated to an absolute URL first, mailto:/tel:/
	 * javascript:/fragment-only links are dropped (nothing to request), and
	 * each surviving target is re-checked against the same private-network
	 * guard as the main URL before it is fetched — see
	 * azwc_audit_link_host_ok(). This samples the first
	 * AZWC_LINKCHECK_MAX links found, not every link on the page; the wording
	 * says so, so a partial check is never read as a full one.
	 */
	$link_targets = array();
	foreach ( $links[1] as $href ) {
		$href = trim( $href );
	...
	if ( $link_sample ) {
		$checks[] = azwc_audit_check(
			...
			sprintf(
				'Checked the first %d link%s found on this page (of %d total)%s: %s.',
				count( $link_sample ),
				1 === count( $link_sample ) ? '' : 's',
				count( $link_targets ),
				$link_skipped ? sprintf( ', %d skipped as pointing at a private/internal address', $link_skipped ) : '',
				empty( $broken_links )
					? 'every one returned a working response'
					: sprintf( '%d did not load correctly', count( $broken_links ) )
			),
			2,
			'onpage',
			$broken_links
...
/**
 * The score from the last time this same host was audited.
 *
 * Keyed on host rather than the full URL, so http/https or a trailing path
 * difference does not silently reset the comparison. Kept for 30 days,
 * separate from the 6-hour result cache, which exists only to spare the
 * origin repeat requests and is far too short-lived for "since last time"
 * to mean anything.
 */
...
function azwc_audit_stage_site( $url ) {
	// The broken-links check adds up to AZWC_LINKCHECK_MAX more outbound
	// requests on top of the page/robots/sitemap fetches already here, so
	// the default execution-time limit is no longer a safe assumption.
...
#azwc-audit .azwc-scale{display:flex;justify-content:space-between;font-size:10px;color:var(--muted);margin-top:-4px;margin-bottom:8px}
	...
	#azwc-audit .azwc-donut-wrap{display:flex;align-items:center;gap:20px;margin-top:16px;flex-wrap:wrap}
	#azwc-audit .azwc-donut-wrap svg text{fill:var(--ink)}
	#azwc-audit .azwc-donut-legend{margin:0;padding:0;list-style:none;display:grid;gap:8px;font-size:13px;color:var(--ink)}
	#azwc-audit .azwc-donut-legend i{display:inline-block;width:11px;height:11px;margin-right:8px;border-radius:3px;vertical-align:-1px}
	#azwc-audit .azwc-donut-legend b{font-weight:800;font-variant-numeric:tabular-nums}
	...
	/* Not background-attachment:fixed — see azw-dark-theme.php for why: it only
	   paints the original viewport height, leaving a visible seam once the
	   audit results grow the page taller. */
	body.azwc-audit-page{background:linear-gradient(135deg,#050608,#111823 60%,#30240a) !important}
	...
	body.azwc-audit-page #azwc-audit .azwc-donut-wrap svg circle:first-of-type{stroke:rgba(255,255,255,.13)}
...
function azwc_audit_script() {
	...
	(function () {
		...
		/**
		 * The pass/warn/fail split as an actual pie chart, not a row of
		 * numbers. Three overlaid circles, each drawn as one dash of a
		 * dashed stroke sized to its share of the total and rotated to
		 * start where the previous slice ended — same ninety-degree
		 * starting convention as gauge() above, so the two visually match.
		 */
		function donut(counts, total) {
			var r = 46, cx = 60, cy = 60, circ = 2 * Math.PI * r;
			var segs = [
				{ n: counts.fail, color: COLOR.fail, label: 'need fixing' },
				{ n: counts.warn, color: COLOR.warn, label: 'worth improving' },
				{ n: counts.pass, color: COLOR.pass, label: 'passing' }
			];
			var seen = 0;
			var arcs = segs.map(function (s) {
				...
				var frac = s.n / total;
				var len = frac * circ;
				var rotate = (seen / total) * 360 - 90;
				seen += s.n;
				return '<circle cx="' + cx + '" cy="' + cy + '" r="' + r + '" fill="none" stroke="' + s.color
					+ '" stroke-width="22" stroke-dasharray="' + len.toFixed(1) + ' ' + (circ - len).toFixed(1)
					+ '" transform="rotate(' + rotate.toFixed(1) + ' ' + cx + ' ' + cy + ')"><title>'
					+ s.n + ' ' + s.label + '</title></circle>';
			}).join('');
			var label = segs.filter(function (s) { return s.n; })
				.map(function (s) { return s.n + ' ' + s.label; }).join(', ');
			return '<svg viewBox="0 0 120 120" width="132" height="132" role="img" aria-label="' + esc(label) + '">'
				+ '<circle cx="' + cx + '" cy="' + cy + '" r="' + r + '" fill="none" stroke="#eceff3" stroke-width="22"/>'
				+ arcs
				+ '<text x="' + cx + '" y="' + (cy + 7) + '" text-anchor="middle" font-size="24" font-weight="700" fill="#111827">' + total + '</text>'
				+ '</svg>';
		}

		function donutBlock(counts) {
			var total = counts.pass + counts.warn + counts.fail;
			if (!total) { return ''; }
			return '<div class="azwc-donut-wrap">' + donut(counts, total)
				+ '<ul class="azwc-donut-legend">'
				+ '<li><i style="background:' + COLOR.fail + '"></i><b>' + counts.fail + '</b> need fixing</li>'
				+ '<li><i style="background:' + COLOR.warn + '"></i><b>' + counts.warn + '</b> worth improving</li>'
				+ '<li><i style="background:' + COLOR.pass + '"></i><b>' + counts.pass + '</b> passing</li>'
				+ '</ul></div>';
		}
   ...
   poor, and here is why that matters". */
		...
		/**
			 * "Since your last check" — only shown when we actually have a prior
			 * score for this host, stored the last time it was audited. Never
			 * estimated or defaulted to zero; no history means no line.
			 */
			function historyLine(h) {
				if (!h || h.previous_score === null || h.previous_score === undefined) { return ''; }
				var delta = h.delta;
				var when = new Date(h.previous_fetched).toLocaleDateString();
				var word = delta > 0 ? ('up ' + delta + ' point' + (delta === 1 ? '' : 's'))
					: (delta < 0 ? ('down ' + Math.abs(delta) + ' point' + (delta === -1 ? '' : 's'))
					: 'unchanged');
				var color = delta > 0 ? COLOR.pass : (delta < 0 ? COLOR.fail : '#9aa3ad');
				return '<p class="azwc-sub" style="margin-top:-8px">Last checked here on ' + esc(when)
					+ ', scoring ' + esc(h.previous_score) + ' &mdash; <strong style="color:' + color + '">'
					+ esc(word) + '</strong> since then.</p>';
			}
		 ...
		 *
		 * All site checks arrive in one response, so this is a staggered
		...
		function run(domain) {
			...
			// 32 is a guess at what the current check set produces (a few checks are
			// conditional); the grid reconciles to the real count as soon as the
			// response arrives, so a drift here is cosmetic.
			scanStart(domain, 32);
			...
			post({ domain: domain, stage: 'site' })
				...
				.then(function () {
					...
					// piece, with every section already populated. The progress panel's
					// job is done once the report is up, so it comes down here rather
					// than sitting on screen above the finished result.
					render(data);
					progress.hidden = true;
					button.disabled = false;
				})
