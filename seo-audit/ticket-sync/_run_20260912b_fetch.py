...
CHECKPOINT_UID = 4079
CHECKPOINT_DT = parsedate_to_datetime("Sat, 12 Sep 2026 18:11:02 GMT")
...
folder_names = []
for f in folders:
    s = f.decode(errors='replace') if isinstance(f, bytes) else str(f)
...
for s in folder_names:
    ...
    if 'sent' in low:
        parts = s.split('"')
        sent_folder = parts[-2] if len(parts) >= 2 else s.split()[-1]
        break

inbox_msgs, inbox_max_uid, inbox_max_date = fetch_inbox(M)

sent_msgs, sent_max_uid, sent_max_date = [], 0, None
sent_folder_used = None
if sent_folder:
    try:
        sent_msgs, sent_max_uid, sent_max_date = fetch_sent(M, sent_folder)
        sent_folder_used = sent_folder
    except Exception as e:
        sent_msgs = []
        sent_folder_used = f"ERROR: {e}"

M.logout()

out = {
    "folders": folder_names,
    "sent_folder_detected": sent_folder_used,
...
with open("_run_20260912b_out.json", "w", encoding="utf-8") as f:
    json.dump(out, f, indent=2, ensure_ascii=False)
print("folders:", folder_names)
print("sent folder detected:", sent_folder_used)
