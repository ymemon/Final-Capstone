...
CHECKPOINT_UID = 8747
CHECKPOINT_DT = parsedate_to_datetime("Mon, 07 Sep 2026 17:36:25 +0000")
...
def fetch_folder(M, folder, checkpoint_uid=None, checkpoint_dt=None):
    ...
    if checkpoint_uid is not None:
        ...
        typ, data = M.uid('search', None, 'SINCE "05-Sep-2026"')
    uids = data[0].split()
    results = []
...
for name in ["Sent", "Sent Items", "INBOX.Sent"]:
    for fn in folder_names:
        if fn.rstrip().endswith(name) or fn.rstrip().endswith(f'"{name}"'):
            sent_folder = name
            break
    if sent_folder:
        break
...
with open("run_20260907_check.json", "w", encoding="utf-8") as f:
    json.dump(out, f, indent=2, ensure_ascii=False)

print(f"Folders: {folder_names}")
print(f"INBOX new: {len(inbox_msgs) if inbox_msgs else 0}, max_uid={inbox_max_uid}")
