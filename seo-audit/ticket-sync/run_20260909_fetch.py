...
def fetch_folder_by_uid(M, folder, checkpoint_uid):
    ...
    uids = data[0].split()
    return extract(M, uids, use_uid=True, checkpoint_uid=checkpoint_uid)

def fetch_folder_by_date(M, folder, checkpoint_dt):
    M.select(folder, readonly=True)
    since_str = checkpoint_dt.strftime('%d-%b-%Y')
    typ, data = M.search(None, f'SINCE "{since_str}"')
    uids = data[0].split()
    return extract(M, uids, use_uid=False, checkpoint_dt=checkpoint_dt)

def extract(M, uids, use_uid, checkpoint_uid=None, checkpoint_dt=None):
    results = []
...
sent_max_date = None
if sent_folder:
    sent_msgs, sent_max_uid, sent_max_date = fetch_folder_by_date(M, sent_folder, CHECKPOINT_DT)
...
with open("run_20260909_fetch_out.json", "w", encoding="utf-8") as f:
