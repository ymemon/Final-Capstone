...
def fetch_folder_by_uid(M, folder, checkpoint_uid):
    ...
    uids = data[0].split()
    return extract(M, uids, checkpoint_uid, use_uid=True)

def fetch_folder_by_date(M, folder, checkpoint_dt):
    M.select(folder, readonly=True)
    typ, data = M.search(None, 'SINCE "05-Sep-2026"')
    uids = data[0].split()
    return extract(M, uids, None, use_uid=False, checkpoint_dt=checkpoint_dt)

def extract(M, uids, checkpoint_uid, use_uid, checkpoint_dt=None):
    results = []
...
with open("sync_20260907_run5.json", "w", encoding="utf-8") as f:
