...
CHECKPOINT_UID = 4081
CHECKPOINT_DT = parsedate_to_datetime("Sat, 12 Sep 2026 19:51:07 +0000")
...
if sent_folder is None:
    for f in folders:
        if 'Sent' in f:
            parts = f.split('"')
            sent_folder = parts[-2] if len(parts) >= 2 else None
...
with open("_fetch_run5_out.json", "w", encoding="utf-8") as f:
    json.dump(out, f, indent=2, ensure_ascii=False)
print("inbox new:", len(inbox_msgs), "sent new:", len(sent_msgs))
print("sent_folder_used:", sent_folder)
