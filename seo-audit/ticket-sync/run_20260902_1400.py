...
typ, data = M.uid('search', None, f'UID {CHECKPOINT_UID+1}:*')
# Sent UIDs are a different UID space; instead use date-based search since checkpoint date to be safe, then filter later by message-id/UID tracking separately.
# We'll just fetch all UIDs and rely on checkpoint date comparison in the analysis step.
typ, data2 = M.search(None, 'SINCE "31-Aug-2026"')
...
with open("run_20260902_1400.json", "w", encoding="utf-8") as f:
